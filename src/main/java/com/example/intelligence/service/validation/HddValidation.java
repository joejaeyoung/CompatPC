...
public class HddValidation {
    ...
    public void checkWithCase(ServiceUserRequest request, Cases cases) {
        ...
        if (request.getHddCount() > cases.getBay3_5Count()) {
            errorMsg.add(new ServiceValidationResponse("케이스에 3.5인치 HDD를 장착할 공간이 부족합니다.", "선택하신 HDD는 총 " + request.getHddCount() + "개이며, 현재 케이스에는 3.5인치 베이가 " + Math.max(cases.getBay3_5Count(), 0) + "개만 제공됩니다.\n" +
