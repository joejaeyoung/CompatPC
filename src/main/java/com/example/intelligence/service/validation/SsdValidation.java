...
public class SsdValidation {
    ...
    public void checkWithCase(ServiceUserRequest request, Cases cases) {
        ...
        if(request.getSatassdCount() > cases.getBay2_5Count()) {
            errorMsg.add(new ServiceValidationResponse("케이스에 2.5인치 SATA SSD를 장착할 공간이 부족합니다.", "선택하신 SATA SSD는 총 " + Math.max(request.getSatassdCount(), 0) + "개이며, 현재 케이스에는 2.5인치 베이가 " + Math.max(cases.getBay2_5Count(), 0)+ "개만 제공됩니다.\n" +
                    "따라서 일부 SSD 장착이 불가능 합니다.\n" + "2.5인치 베이를 " + Math.max(request.getSatassdCount(), 0) + "개 이상 제공하는 케이스로 변경하거나 SATA SSD 수량을 줄여주세요.\n" +
